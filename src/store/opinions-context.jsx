...
export function OpinionsContextProvider({ children }) {
  ...
  async function loadUserOpinions(userName) {
    const response = await fetch(
      `http://localhost:3001/opinions/byUser/${userName}`
    );
  }

  async function loadOpinion(id) {
    const response = await fetch(`http://localhost:3001/opinions/${id}`);
  }
  ...
  async function upvoteOpinion(id) {
    const response  = await fetch(`http://localhost:3001/opinions/${id}/upvote`,{
      method: "POST",
    })
  ...
  async function downvoteOpinion(id) {
    const response  = await fetch(`http://localhost:3001/opinions/${id}/downvote`,{
      method: "POST",
    })
