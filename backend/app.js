...
async function loadUserOpinions(userName) {
  try {
    const dbFileData = await fs.readFile("./db.json");
    const parsedData = JSON.parse(dbFileData);
    return parsedData.opinions.filter(
      (opinion) => opinion.userName === userName
    );
  } catch (error) {
    return [];
  }
...
async function loadOpinion(id) {
  try {
    const dbFileData = await fs.readFile("./db.json");
    const parsedData = JSON.parse(dbFileData);
    return parsedData.opinions.find((opinion) => opinion.id == id);
  } catch (error) {
    return {};
  }
}
...
app.get("/opinions/byUser/:userName", async (req, res) => {
  const { userName } = req.params;
  try {
    const opinions = await loadUserOpinions(userName);
    res.json(opinions);
  } catch (error) {
    res.status(500).json({ error: "Error loading user opinions." });
  }
});
...
app.get("/opinions/:id", async (req, res) => {
  const { id } = req.params;
  await new Promise((resolve) => setTimeout(resolve, 1000));
  try {
    const opinion = await loadOpinion(Number(id));
    if (!opinion) {
